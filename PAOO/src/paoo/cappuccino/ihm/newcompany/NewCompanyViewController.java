...
public class NewCompanyViewController extends JPanel implements
  ...
  public NewCompanyViewController(NewCompanyModel model, MenuModel menu,
    ...
    wrapperPanel.setBorder(new EmptyBorder(IhmConstants.L_GAP,
    ...
    wrapperPanel.add(new NewCompanyView(model, companyNameField, numField,
    ...
    createButton
        .addActionListener(e -> {
          // test input
          String nameError =
              StringUtils.isEmpty(companyNameField.getText()) ? IhmConstants.ERROR_FIELD_EMPTY
                  : companyNameField.getText().length() >= 30 ? IhmConstants.ERROR_TOO_LONG
                      : null;

          String streetError =
              StringUtils.isEmpty(streetField.getText()) ? IhmConstants.ERROR_FIELD_EMPTY
                  : null;

          String cityError =
              StringUtils.isEmpty(cityField.getText()) ? IhmConstants.ERROR_FIELD_EMPTY
                  : null;

          String boxError =
              StringUtils.isEmpty(boxField.getText()) ? null
                  : (StringUtils.isAlphanumeric(boxField.getText()) ? null
                      : IhmConstants.ERROR_ALPHANUM_INPUT);

          String numError =
              StringUtils.isEmpty(numField.getText()) ? IhmConstants.ERROR_FIELD_EMPTY
                  : null;

          String postCodeError =
              StringUtils.isEmpty(postCodeField.getText()) ? IhmConstants.ERROR_FIELD_EMPTY
                  : ((!StringUtils.isNumeric(postCodeField.getText()) || postCodeField
                      .getText().length() != 4) ? "Le code postal doit être composé de 4 chiffres"
                      : null);

          model.setErrors(nameError, streetError, cityError, boxError,
              numError, postCodeError);
          if (model.hasError()) {
            return;
          }
