...
class CompanyDao implements ICompanyDao {
  ...
  public ICompanyDto[] fetchInvitableCompanies() {
    ...
    String query = "SELECT c.company_id, c.creator, c.name, c.register_date,"
                   + " c.address_street, c.address_num, c.address_mailbox, c.address_postcode,"
                   + " c.address_town, c.companies.version "
                   + "FROM business_days.companies c "
                   + "WHERE EXISTS ("
                   + "  SELECT p.* FROM business_days.participations p, "
                   + "                  business_days.business_days d"
                   + "  WHERE c.company_id = p.company"
                   + "   AND p.business_day = d.business_day_id"
                   + "   AND p.state = 'PAID'"
                   + "   AND (("
                   + "     date_part('month', now()) between 1 AND 8 "
                   + "     AND date_part('years', now() - INTERVAL '4 year') < "
                   + "      date_part('year', d.event_date)"
                   + "    ) OR ("
                   + "     date_part('month', now()) between 9 AND 12 "
                   + "     AND date_part('years', now() - INTERVAL '4 year') <"
                   + "      date_part('year', d.event_date)"
                   + "    )"
                   + "UNION \n"
